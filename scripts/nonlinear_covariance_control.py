...
from i2c.utils import make_results_folder, configure_plots, covariance_2d
...
def plot_covariance_control(i2c, xs, filename="", dir_name=None):
    ...
    a = ax[0]
    a.set_title("Pendulum Covariance Control")
    ...
    a.plot(np.log(kl_term), "kx-", markersize=1)
    a.set_ylabel("log KL$(\\mathbf{x}_T||\\mathbf{x}^*_T)$")
